...
- Поиск и скачивание с YouTube (сервер `tube/`), прослушивание превью стримом.
...
`tube/` (YouTube), `tg/` (Телеграм-бот), загрузка файла с устройства. Общий вход
...
```
юзер пересылает аудио боту → сервер (tg/api) кладёт в очередь МЕТАДАННЫЕ и file_id
...
запроса требует api-секрет last.fm, поэтому она на сервере (`scrobble/api`):
...
- **Сервер приложения**: один процесс на `tube.87.120.36.150.ip.giper.dev`
  (`tube/deploy/`), в нём три набора маршрутов: `tube` (yt-dlp), `tg`
  (телеграм-бот), `fm` (скробблинг). Второй node на бокс не ставим: там 1 CPU и
  ~960МБ на всё вместе с baza. Переменные окружения — в `tube/deploy/compose.yml`.
