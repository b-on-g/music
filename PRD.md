...
- Поиск и скачивание с YouTube (сервер `srv/tube`), прослушивание превью стримом.
- Приём музыки из Телеграма: пересланное боту приезжает в «Мою музыку».
- Скробблинг прослушанного в last.fm.
...
- Перемотка тапом и перетаскиванием по прогресс-бару; ползунок в rem, растёт
  вместе с размером шрифта.
- Персональный обрез трека (trim start/end) — ручки появляются по кнопке
  «ножницы» в полном плеере, чтобы не путаться с перемоткой.
- Обложки: YouTube-превью и альбом VK хранятся в треке (`Cover`) и показываются
  в списке и в плеере.
- Удаление навсегда — только через подтверждение прямо в строке трека.
...
Источники треков (в baza не живут, только приносят байты): `api/`+`hls/` (VK),
`tube/` (YouTube), `tg/` (Телеграм-бот), загрузка файла с устройства; серверные
половины трёх источников — в `srv/`. Общий вход
у всех один — `account.import_audio(audio, bytes, mime)`.
...
- `tracks/`, `track/` — список и строка трека. Строка получает только `key`
  и читает метаданные из домена. Действия строки — в меню «⋯», чтобы тексту
  доставалась вся ширина на крупном шрифте.
- `pop/` — открытие/закрытие `$mol_pop` тапом и закрытие тапом мимо (на
  телефоне `$mol_pop_over` не закрывается: фокус остаётся на кнопке).
- `landing/` — `$bog_music_landing`: отдельный лёгкий бандл-лендинг для
  поиска, без плеера и baza. В деплое пререндерится
  (`b-on-g/mol-prerender-action`) и уезжает в `/music/landing/`.
...
### Поток «трек из Телеграма»

```
юзер пересылает аудио боту → сервер (srv/tg) кладёт в очередь МЕТАДАННЫЕ и file_id
       → приложение раз в 30 с спрашивает /tg/status
       → /tg/inbox → /tg/file (сервер тянет байты из Телеграма и проксирует)
       → домен.import_audio() → /tg/ack
```

Связка устройства с чатом — по коду (`bog/music/code`, 96 бит в localStorage):
он же уходит в диплинк `t.me/<бот>?start=<код>`, он же авторизует чтение очереди.
Байты на сервере не хранятся вообще: в очереди только метаданные, файл живёт в
Телеграме и тянется в момент запроса. Дедуп по `file_unique_id` — один и тот же
трек, пересланный дважды, не задваивается. Потолок файла — 20 МБ (столько Bot API
отдаёт ботам).

### Поток «скробблинг»

Плеер на каждом тике зовёт `scrobble_watch()`: смену трека ловит по метаданным
(не по событию — в `play_track` лишний код рвёт фоновый звук на iOS), шлёт
`now playing`, а после «больше половины или больше 4 минут» — скробл. Подпись
запроса требует api-секрет last.fm, поэтому она на сервере (`srv/fm`):
клиент знает только имя юзера, ключей и session key не видит.
...
- **Сервер приложения**: один процесс на `tube.87.120.36.150.ip.giper.dev`
  (`srv/deploy/`), в нём три набора маршрутов: `tube` (yt-dlp), `tg`
  (телеграм-бот), `fm` (скробблинг). Второй node на бокс не ставим: там 1 CPU и
  ~960МБ на всё вместе с baza. Переменные окружения — в `srv/deploy/compose.yml`,
  секреты на боксе в `.env` рядом с ним. Бандл смонтирован томом, а не запечён в
  образ (диск под завязку, пересборка гоняет apt с ffmpeg): обновление = scp
  `node.js` в `/root/tube/` + `docker compose up -d --force-recreate`. Без
  `--force-recreate` контейнер продолжает читать старый inode примонтированного
  файла и код не обновляется.
- **Серверный бандл держать чистым от браузерного кода.** mam подключает к
  модулю его родителя, поэтому серверные половины лежат в `srv/`, а не рядом с
  клиентами: иначе в node-бандл затягивает `$mol_fetch`/`$mol_state_local`, а
  те тянут jsdom, которого в образе нет — контейнер падает на старте.
...
- **Билд**: из корня MAM — `npx mam bog/music/app` (приложение) и `npx mam bog/music/srv/tube` (сервер), артефакты в `-/`.
