...
- Выравнивание громкости треков по EBU R128 (тумблер в панели громкости).
...
- `gain/` — замер громкости записи в LUFS по EBU R128 (K-взвешивание + гейтинг)
  и множитель приведения к -14 LUFS. Замер одноразовый, ленивый (при первом
  проигрывании и заранее для следующего трека), результат живёт в поле `Lufs`
  трека и синкается между устройствами. Играет всё через цепочку
  `<audio> → gain → limiter`: el.volume для этого не годится — на iOS он
  игнорируется системой, на остальных зажат единицей и не умеет усиливать.
...
- Громкость выравнивается только у скачанных треков: замер идёт по блобу, а
  стриму (превью с tube, HLS-фолбэк) мерить нечего. Стрим при включённой
  цепочке требует CORS-заголовков от источника — WebAudio молчит на чужом
  домене без них.
